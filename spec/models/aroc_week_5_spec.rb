...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.joins(:order_items).distinct(:id).order(:name).pluck(:name)
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select('users.name, COUNT(order_items.id) AS total_item_count').joins(:order_items).group(:name).order('users.name')
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    data = User.select('users.name AS user_name, orders.id AS order_id, orders.amount / COUNT(order_items.id) AS avg_item_cost').joins(orders: :order_items).group(:order_id).order('user_name DESC, avg_item_cost')
    # User.joins(orders: :order_items).group(:order_id).average(:amount)
    # User.select('user.name AS user_name, orders.id AS order_id, orders.amount / COUNT(order_items.id) AS avg_item_cost').joins(orders: :order_items).group(:order_id)
