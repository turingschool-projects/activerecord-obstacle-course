...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.joins(:orders).select("items.name").distinct.order(:name).pluck(:name)
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = Order.joins(:user).group("users.name").select("users.name, COUNT(*) AS total_order_count").order("total_order_count")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:order_items).group("users.name").select("users.name, COUNT(*) AS total_item_count")
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(:order_items).select("users.name AS user_name").select("orders.id AS order_id").select("orders.amount / count(order_items.id) AS avg_item_cost").group("user_name, order_id").order("user_name DESC, avg_item_cost")
    # ---------------------------------------------------------------
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.includes(:order_items).includes(:items)
    # ------------------------------------------------------
