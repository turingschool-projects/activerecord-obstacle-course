...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # Solution goes here
    ordered_items_names = Item.distinct.joins(:order_items)
                              .joins(:orders)
                              .pluck(:name)
                              .sort
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.select('users.name AS user_name, orders.id AS order_id, orders.amount / COUNT(order_items.item_id) AS avg_item_cost')
               .joins(:order_items)
