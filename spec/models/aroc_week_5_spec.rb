...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.left_outer_joins(:order_items).where("order_items.item_id").distinct.pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select("users.name, count(orders.id) as total_order_count").group("users.id").order("total_order_count")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    # custom_results = User.joins(:orders).joins(:items).select("users.name, count(orders.item_id) as total_item_count").group("users.name").order("users.name")
    custom_results = User.joins(:order_items).select("users.name, count(order_items.item_id) as total_item_count").group("users.id").order("users.name")
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # data = User.joins(:order_items).joins(:orders).joins(:items).group("orders.id").select("users.name, orders.id as order_id").select("count(order_items.item_id) as item_count").order
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = Order.joins(:user).joins(:items).group("orders.id").select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.item_id) as avg_item_cost").order("user_name desc, avg_item_cost asc")
    # ---------------------------------------------------------------
