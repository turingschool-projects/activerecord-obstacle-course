...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    unordered_items = Item.joins(:orders).distinct.order(:name).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    custom_results = [@user_3, @user_2, @user_1]
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    data = User.select("name AS user_name, order_items.order_id AS order_id, orders.amount / count(order_items.id) AS avg_item_cost")
               .joins(:order_items)
