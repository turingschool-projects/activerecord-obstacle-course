...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items = Item.joins(:order_items).where.not("order_items.item_id = id")    
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = Order.joins(:user).select("users.name", "COUNT(orders.id) AS total_order_count").group(:name).order("total_order_count")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(order_items: :item).select("name", "COUNT(order_items.id) AS total_item_count").group(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: :items).select("users.name AS user_name", "orders.id AS order_id", "orders.amount / COUNT(order_items.id) AS avg_item_cost")
                .group("users.name, orders.id").order("user_name DESC", "avg_item_cost ASC")
    # ---------------------------------------------------------------
