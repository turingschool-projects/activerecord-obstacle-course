...
# ----------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:order_items).select('users.*, count(order_items.id) AS total_item_count').group(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    data = Order.joins(:user, :order_items).select('orders.*, orders.id AS order_id, users.name AS user_name, orders.amount / COUNT(order_items.item_id) AS avg_item_cost').group('orders.id').order("users.name DESC, avg_item_cost ASC")
    # ---------------------------------------------------------------
