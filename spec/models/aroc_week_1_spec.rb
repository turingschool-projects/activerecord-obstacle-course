...
# ----------------------
  ...
  it '2. finds order id of smallest order' do
    # active record order defaults to ascending order
    # what is syntax to specify asc vs dec
    # is there a way to rewrite with find to remove id call finishing chain or is this the best syntax
    # ----------------------- Using Raw SQL ----------------------
  ...
  it '4. finds orders of multiple amounts' do
    ...
    # end
    # ------------------------------------------------------------
    orders_of_500_and_700 = ActiveRecord::Base.connection.execute('select * from orders where amount = 500 or amount = 700;')
    orders_of_700_and_1000 = ActiveRecord::Base.connection.execute('select * from orders where amount = 700 or amount = 1000;')
    # ------------------ Using ActiveRecord ----------------------
  ...
  it '6. finds multiple orders by id' do
    ...
    # ------------------------------------------------------------
    #correct but fails expectpectation bc expectation is ORM objects
    orders = ActiveRecord::Base.connection.execute("select * from orders where id = #{@order_1.id} or id = #{@order_2.id} or id = #{@order_3.id} or id = #{@order_4.id};")
    # ------------------ Using ActiveRecord ----------------------
    orders = Order.where(id: [5, 1, 3, 14])
    # ------------------------------------------------------------
