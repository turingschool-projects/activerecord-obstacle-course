...
# ----------------------
  ...
  it '3. finds order id of largest order' do
    ...
    # Solution goes here
    # Your solution should not contain the ID of the order anywhere
    # order_id = Order.order(amount: :desc).limit(1).id
    # order_id = Order.order(amount: :desc).first.id
    # order_id = Order.order(:amount).reverse_order.limit(1).pluck(:id).first
  ...
  it '4. finds orders of multiple amounts' do
    ...
    orders_of_700_and_1000 = Order.where(amount: 700).or(Order.where(amount:700))
  ...
  it '6. finds multiple orders by id' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # This returns the array in the wrong order: orders = Order.find(ids_to_find)
  ...
  it '7. finds orders with an amount between 700 and 1000' do
    ...
    orders_between_700_and_1000 = Order.where(amount: 700...1001)
  ...
  it '8. finds orders with an amount less than 550' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # strong params: orders_less_than_550 = Order.where("amount < ?", 550)
