...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:orders, :order_items).where("order_items.item_id=#{@item_8.id}").select('users.name').distinct.order('users.name').pluck(:name)
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = Order.where("orders.user_id = #{@user_3.id}").average(:amount)
    # ------------------------------------------------------------
