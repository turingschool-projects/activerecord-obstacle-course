...
# ----------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = User.where("users.id != 2").joins(:orders).sum("orders.amount")
    # -----------------------------------------------------------
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders =  Order.joins(:items).where("items.name = 'Banana Republic'")
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:items).joins(:user).where("users.name = 'Hansel' and items.name = 'Banana Republic'")
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    ordered_items = Item.left_outer_joins(:order_items).where("order_items.item_id").distinct
