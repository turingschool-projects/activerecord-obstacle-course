...
# ----------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = Order.where("orders.user_id != #{@user_2.id}").sum(:amount)
    # -----------------------------------------------------------
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).select('orders.*').where("order_items.item_id = #{@item_4.id}")
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where("orders.user_id = #{@user_2.id} AND order_items.item_id = #{@item_4.id}").select('orders.*')
    # -----------------------------------------------------------
