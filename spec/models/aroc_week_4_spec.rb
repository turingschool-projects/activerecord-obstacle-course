...
# ----------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    id_to_exclude = @user_2.id
    total_sales = Order.where('user_id != ?', id_to_exclude).sum(:amount)
    # -----------------------------------------------------------
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    item_id_to_find = @item_4.id
    orders = Order.joins(:order_items).where('order_items.item_id = ?', item_id_to_find)
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    item = @item_4.id
    user = @user_2.id
    orders = Order.joins(:order_items).where('order_items.item_id = ?', item).where('user_id = ?', user)
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    ordered_items = Item.joins(:orders).distinct(:item).order(:name)
    # ---------------------------------------------------------------
